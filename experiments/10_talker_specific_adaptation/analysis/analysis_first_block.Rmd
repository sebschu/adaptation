...
trials = remove_quotes(format_data(read.csv("../data/10_talker_specific_adaptation-trials.csv")))
...
plot_conditions = function(d, plot_title) {
  ...
  group_by(workerid, percentage_blue, modal, block, test_order, first_speaker_type) %>% 
  summarise(participant_mean = mean(rating)) %>%
  group_by(percentage_blue, modal, block, first_speaker_type) %>%
  summarise(mu = mean(participant_mean),
         ci_high = ci.high(participant_mean), 
         ci_low = ci.low(participant_mean))   %>% 
  filter(modal != "other")
  ...
  p5 = ggplot(d_means.first_speaker_type, aes(x=percentage_blue, y=mu, col=modal, lty=factor(block))) + 
  xlab("event probability") +
  ylab("mean ratings") +
  geom_errorbar(aes(ymin=mu-ci_low, ymax=mu+ci_high), width=5) +
  geom_line() +
  ...
  return(list("p1" = p1, "p2" = p2, "by_participant" = by_participant, "p3" = p3, "p4" = p4, "p5"= p5))
