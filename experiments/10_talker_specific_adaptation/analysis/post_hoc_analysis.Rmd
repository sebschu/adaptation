...
plot_conditions = function(d, plot_title) {
  ...
  d_means = d %>%
  group_by(workerid, percentage_blue, modal, first_speaker_type) %>% 
  summarise(participant_mean = mean(rating)) %>%
  group_by(percentage_blue, modal, first_speaker_type) %>%
  summarise(mu = mean(participant_mean),
         ci_high = ci.high(participant_mean), 
         ci_low = ci.low(participant_mean))

  
  d_means$first_speaker_type = factor(d_means$first_speaker_type, labels = c("cautious", "confident"))
  
  p1 = d_means %>% filter(modal != "other") %>% ggplot(aes(x=percentage_blue, y=mu, col=modal)) + 
  ...
  plot(ps1$p1)
    ggsave(filename = "exp2-results.pdf", plot = ps1$p1, device = "pdf", width=5, height=4)
...
aucs.cautious = d %>% filter(first_speaker_type == "cautiousfirst") %>% auc_for_participants(., method=auc_method2)

#AUCs for confident speaker ratings 
aucs.confident = d %>% filter(first_speaker_type == "confidentfirst") %>% auc_for_participants(., method=auc_method2)
...
model = lm(formula= auc_diff ~ cond + test_order + confident_speaker, data = rbind(aucs.cautious, aucs.confident))
