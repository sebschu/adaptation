...
plot_conditions = function(d, plot_title) {
  ...
  d_means = d %>%
  group_by(workerid, percentage_blue, modal, first_speaker_type,test_order, speaker_type) %>% 
  summarise(participant_mean = mean(rating)) %>%
  group_by(percentage_blue, modal, first_speaker_type, test_order, speaker_type) %>%
  ...
  ggtitle(plot_title) +
  facet_wrap(~first_speaker_type+test_order)
...
model = lm(formula= auc_diff ~ cond + test_order + first_speaker_type*test_order + confident_speaker, data = rbind(aucs.cautious, aucs.confident))
