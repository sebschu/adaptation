...
p1a = ggplot(d_comparison %>% group_by(modal,percentage_blue, pair) %>% summarise(rating_m = mean(rating), ci_low=ci.low(rating), ci_high=ci.high(rating)), aes(x=percentage_blue, y=rating_m, group=modal, col=modal)) + geom_point(aes(col=modal), size=4) + geom_line(size=2) + xlab("percentage") +   geom_errorbar(aes(ymin=rating_m-ci_low, ymax=rating_m+ci_high), width=5, size=1) + ylab("mean rating") + facet_wrap(~pair) + colscale   + guides(col=guide_legend(title="Expression", nrow = 1)) + theme(legend.position="bottom", legend.text=element_text(size=14))
...
```{r echo=FALSE, fig.width=10, fig.height=3.125}
...
participant_plot = ggplot(ps_old$d %>% 
  ...
  geom_point(aes(col=modal), size=3) + 
  geom_line(size=2) + 
  xlab("event probability") + 
