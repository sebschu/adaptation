...
var post_trial_idxs = _.range(REPETITIONS * percentages.length * colors.length * 2);

var post_catch_trial_idxs = _.sample(post_trial_idxs, 9);

var phase = "pre_exposure";

var SPEAKER_REFS = {"m": "man", "f": "woman"};
...
function build_trials() {
	...
	for (var i = 0; i < REPETITIONS; i++) {
        ...
        for (var l = 0; l < speakers.length; l++) {
  				...
  				trials.push({
  					"catch_trial": (post_catch_trial_idxs.indexOf(t_idx) > -1) ? 1 : 0,
  					"pair": cond["pair"].join("-"),
...
function make_slides(f) {
  ...
  var present_handle_trial = function(stim) {
    ...
    $("#speaker-ref").text(SPEAKER_REFS[stim.speaker]);
    ...
    this.response = response;
      ...
      $(".err").show();
			...
			} else if ( ! this.stim.catch_trial == 1 || this.step == 1) {
				this.step = 0;
	    	this.log_responses();
				var t = this;
    ...
    exp.data_trials.push({
      ...
      "modal2" : sent2[0],
			...
			"catch_trial": this.stim.catch_trial,
  ...
  slides.thanks = slide({
    ...
    start : function() {
      ...
      setTimeout(function() {
        ...
        formdata = {"data": JSON.stringify(exp.data)};
        ...
        $.post("https://stanford.edu/~sebschu/cgi-bin/ling-145-submit-adaptation.php", formdata, success_fct);
...
function init() {
  ...
  exp.exp_trials = _.shuffle(exposure_trials[CONDITION]); 
	exp.pre_trials = _.shuffle(build_trials());
	exp.post_trials = _.shuffle(build_trials());
  exp.catch_trials = [];
  exp.system = {
  ...
  exp.structure=["i0", "instructions", "pre_trial", "exp_instructions", "exp_trial", "test_instructions", "post_trial", 'subj_info', 'thanks'];
