...
```{r per_cond_norm, echo=FALSE, fig.width=10} 
 ...
 ggplot(d.plot %>% group_by(modal,percentage_blue, cond) %>% summarise(rating_m = mean(rating_norm), ci_low=ci.low(rating_norm), ci_high=ci.high(rating_norm)), aes(x=percentage_blue, y=rating_m, group=modal, col=modal)) + geom_point(aes(col=modal)) + geom_line() + xlab("percentage") +   geom_errorbar(aes(ymin=rating_m-ci_low, ymax=rating_m+ci_high), width=.1) + #+ scale_color_manual(values = wes_palette("IsleofDogs1",3)) +
  guides(col=guide_legend(title="Modal")) + ylab("normalized rating") +  theme(text = element_text(size=20)) +
    facet_wrap(~cond) 
