...
d.first_block = d %>%
   filter((first_speaker_type == "cautiousfirst"  & test_order == "parallel" & speaker_type == "cautious") | 
          (first_speaker_type == "confidentfirst" & test_order == "reverse" & speaker_type == "cautious") |
           (first_speaker_type == "confidentfirst" & test_order == "parallel" & speaker_type == "confident")|
           (first_speaker_type == "cautiousfirst" & test_order == "reverse" & speaker_type == "confident")  )

d.first_block$block = 1


d.second_block = d %>%
   filter((first_speaker_type == "cautiousfirst"  & test_order == "reverse" & speaker_type == "cautious") | 
          (first_speaker_type == "confidentfirst" & test_order == "parallel" & speaker_type == "cautious") |
           (first_speaker_type == "confidentfirst" & test_order == "reverse" & speaker_type == "confident")|
           (first_speaker_type == "cautiousfirst" & test_order == "parallel" & speaker_type == "confident")  )

d.second_block$block = 2

d = rbind(d.first_block, d.second_block)
...
```{r aggregate_plots, echo=FALSE, fig.width=10, fig.height=4.5}
...
plot_conditions = function(d, plot_title) {
  ...
  d_means = d %>%
  group_by(workerid, percentage_blue, modal, speaker_type, first_speaker_type, test_order, block) %>% 
  summarise(participant_mean = mean(rating)) %>%
  group_by(percentage_blue, modal, speaker_type, first_speaker_type, block) %>%
  ...
  d_means.block = d %>%
  group_by(workerid, percentage_blue, modal, speaker_type, test_order, block) %>% 
  summarise(participant_mean = mean(rating)) %>%
  group_by(percentage_blue, modal, speaker_type, block) %>%
  summarise(mu = mean(participant_mean),
         ci_high = ci.high(participant_mean), 
         ci_low = ci.low(participant_mean))   %>% 
  filter(modal != "other")
  ...
  d_means.first_speaker_type_block = d %>%
  group_by(workerid, percentage_blue, modal, speaker_type, test_order, first_speaker_type, block) %>% 
  summarise(participant_mean = mean(rating)) %>%
  group_by(percentage_blue, modal, speaker_type, first_speaker_type, block) %>%
  summarise(mu = mean(participant_mean),
         ci_high = ci.high(participant_mean), 
  ...
  d_means.first_speaker_type = d %>%
  group_by(workerid, percentage_blue, modal, speaker_type, test_order, first_speaker_type) %>% 
  summarise(participant_mean = mean(rating)) %>%
  group_by(percentage_blue, modal, speaker_type, first_speaker_type) %>%
  summarise(mu = mean(participant_mean),
         ci_high = ci.high(participant_mean), 
         ci_low = ci.low(participant_mean))   %>% 
  filter(modal != "other")
  ...
  p1 = ggplot(d_means, aes(x=percentage_blue, y=mu, col=modal, lty=first_speaker_type)) + 
  xlab("event probability") +
  ylab("mean ratings") +
  geom_errorbar(aes(ymin=mu-ci_low, ymax=mu+ci_high), width=5) +
  geom_line() +
  geom_point(size=1) +
  guides(col=guide_legend(title="Expr."), lty=guide_legend(title="Speaker")) +
  colscale +
  theme(legend.position="bottom", legend.box = "horizontal") + 
  ggtitle(plot_title) + facet_wrap(~block + speaker_type )

  
   

  p3 = ggplot(d_means.block, aes(x=percentage_blue, y=mu, col=modal, lty=speaker_type)) + 
  xlab("event probability") +
  ylab("mean ratings") +
  geom_errorbar(aes(ymin=mu-ci_low, ymax=mu+ci_high), width=5) +
  geom_line() +
  ...
  colscale +
  theme(legend.position="bottom", legend.box = "horizontal") + 
  ggtitle(plot_title) + facet_wrap(~block )

  
  p4 = ggplot(d_means.first_speaker_type_block, aes(x=percentage_blue, y=mu, col=modal, lty=speaker_type)) + 
  xlab("event probability") +
  ylab("mean ratings") +
  geom_errorbar(aes(ymin=mu-ci_low, ymax=mu+ci_high), width=5) +
  geom_line() +
  ...
  colscale +
  theme(legend.position="bottom", legend.box = "horizontal") + 
  ggtitle(plot_title) + facet_wrap(~block + first_speaker_type )
   
  
    
  p5 = ggplot(d_means.first_speaker_type, aes(x=percentage_blue, y=mu, col=modal, lty=speaker_type)) + 
  xlab("event probability") +
  ylab("mean ratings") +
  geom_errorbar(aes(ymin=mu-ci_low, ymax=mu+ci_high), width=5) +
  geom_line() +
  ...
  colscale +
  theme(legend.position="bottom", legend.box = "horizontal") + 
  ggtitle(plot_title) + facet_wrap(~first_speaker_type )
   

  p6 = ggplot(d_means.all, aes(x=percentage_blue, y=mu, col=modal)) + 
  xlab("event probability") +
  ylab("mean ratings") +
  geom_errorbar(aes(ymin=mu-ci_low, ymax=mu+ci_high), width=5) +
  geom_line() +
  ...
  return(list("p1" = p1, "p2" = p2, "by_participant" = by_participant, "p3" = p3, "p4" = p4, "p5"= p5, "p6" = p6))
  ...
  print("Exposure speaker x block x first speaker type")
  plot(ps1$p1)

  
  print("Block x exposure speaker")
  plot(ps1$p3)

  print("Block x first speaker type")
  plot(ps1$p4)
  ...
  ggsave(filename = "talker-specific-adaptation-results.pdf", plot = ps1$p1, device = "pdf", width=5, height=4)
```

```{r p6, echp=FALSE, fig.width=10, fig.height=4.5}
...
```{r indiv_plots, echo=FALSE, fig.width=10, fig.height=20, cache=TRUE}
...
d$speaker_type = factor(d$speaker_type)
d$test_order = factor(d$test_order)
d$first_speaker_type = factor(d$first_speaker_type)


#AUCs for cautious speaker ratings 
aucs.cautious = d %>% 
  filter(speaker_type == "cautious") %>%  auc_for_participants(., method=auc_method2)
#  filter(  (first_speaker_type == "cautiousfirst" & test_order == "parallel") | 
#           (first_speaker_type == "confidentfirst" & test_order == "reverse")) %>%
 
#AUCs for confident speaker ratings 
aucs.confident = d %>% 
  filter(speaker_type == "confident") %>%   auc_for_participants(., method=auc_method2)

#  filter(  (first_speaker_type == "cautiousfirst" & test_order == "reverse") | 
       #    (first_speaker_type == "confidentfirst" & test_order == "parallel")) %>% 
...
print(cohen.d(aucs.cautious$auc_diff, aucs.confident$auc_diff))

aucs_merged = merge(aucs.cautious, aucs.confident, by=c("workerid"))

x = rbind(aucs.cautious, aucs.confident)
x$block = factor(x$block)

model = lm(formula= auc_diff ~ cond + test_order + first_speaker_type + confident_speaker + block, data = rbind(aucs.cautious, aucs.confident))


print(summary(model))


model2 = lm(formula= auc_diff ~ cond, data = x)
