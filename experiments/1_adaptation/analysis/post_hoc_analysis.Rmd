...
## Analysis 2: Speaker gender differences?

### Catch trial performance

```{r catch_trials_2, echo=FALSE}
...
```{r perf_plot_1_2, fig.height=2,fig.width=10, echo=FALSE}
...
```{r perf_plot_2_2, fig.height=2,fig.width=10, echo=FALSE}
...
```{r auc_method_2_2, echo=FALSE }

#AUCs for might-biased condition
aucs.might_f = auc_for_participants(d.might %>% filter(., speaker_cond == "f"), method=auc_method2)

aucs.might_m = auc_for_participants(d.might %>% filter(., speaker_cond == "m"), method=auc_method2)


#AUCs for probably-biased condition
aucs.probably_f = auc_for_participants(d.probably %>% filter(., speaker_cond == "f"), method=auc_method2)

aucs.probably_m = auc_for_participants(d.probably %>% filter(., speaker_cond == "m"), method=auc_method2)

```

```{r auc_method2_plots_2, fig.width=4, fig.height=4, echo=FALSE}

aucs.might_f$cond = "might-biased"
aucs.might_m$cond = "might-biased"

aucs.probably_f$cond = "probably-biased"
aucs.probably_m$cond = "probably-biased"

aucs.might_f$speaker = "f"
aucs.probably_f$speaker = "f"

aucs.might_m$speaker = "m"
aucs.probably_m$speaker = "m"


aucs.all = rbind(aucs.might_f, aucs.probably_f, aucs.might_m, aucs.probably_m)
aucs.all = aucs.all %>% 
  group_by(., cond, speaker) %>% 
  summarise(., auc_diff_m = mean(auc_diff), 
               ci_high = ci.high(auc_diff), 
               ci_low = ci.low(auc_diff))
...
ggplot(aucs.all, aes(x=speaker, y=auc_diff_m, color=cond)) +
    geom_errorbar(aes(ymin=auc_diff_m-ci_low, ymax=auc_diff_m+ci_high), width=.1) +
    geom_point() +
    xlab("") +
    ylab("AUC difference (might ratings - probably ratings)") +
...
Difference between male and female speaker? Interactions?

```{r lm_gender_might, echo=FALSE} 
...
Differences between male and female speaker in use of "other" for maximally certain?

```{r lm_gender_other, echo=FALSE} 


summary(lm(rating_other ~ speaker_cond, data=d.might %>% filter(., percentage_blue > 99)))

summary(lm(rating_other ~ speaker_cond, data=d.probably %>% filter(., percentage_blue > 99)))
