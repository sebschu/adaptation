...
plot_condition = function(d, plot_title) {
  ...
  d_comparison$modal = factor(d_comparison$modal, levels=c("might", "probably", "other"), ordered = T)
  
  p1 = ggplot(d_comparison %>% group_by(modal,percentage_blue) %>% summarise(rating_m = mean(rating), ci_low=ci.low(rating), ci_high=ci.high(rating)), aes(x=percentage_blue, y=rating_m, group=modal, col=modal)) + geom_point(aes(col=modal, alpha=.5)) + geom_line() + ggtitle(d$pair[1]) + xlab("percentage") +  theme(legend.position="none") +   geom_errorbar(aes(ymin=rating_m-ci_low, ymax=rating_m+ci_high), width=.1) +ylab("rating")  + theme(text = element_text(size=20)) # +geom_area(aes(y=rating_m, fill=modal, alpha=.5, group=modal)) #+ scale_color_manual(values = wes_palette("IsleofDogs1",3))
...
ggplot(aucs.all, aes(x=0, y=auc_diff_m, color=cond)) +
    ...
    geom_point() +
    xlab("") +
    ylab("AUC might  - AUC probably") +
    theme(axis.ticks=element_blank(), axis.text.x=element_blank(),
          panel.grid.minor=element_blank(), 
          plot.background=element_blank()) +
    xlim(-.2, .2) + theme(text = element_text(size=20)) +
