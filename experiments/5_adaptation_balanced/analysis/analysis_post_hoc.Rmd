---
title: "Analysis of adaptation experiment (with balanced exposures)"
...
knitr::opts_chunk$set(root.dir = "/Users/sebschu/Dropbox/Uni/RA/adaptation/adaptation/experiments/5_adaptation_balanced/analysis/")
```
...
trials.might = read.csv("../data/5_adaptation_balanced-might-trials.csv")
exp_trials.might = read.csv("../data/5_adaptation_balanced-might-exp_trials.csv")


# Probably-biased condition
trials.probably = read.csv("../data/5_adaptation_balanced-probably-trials.csv")
exp_trials.probably = read.csv("../data/5_adaptation_balanced-probably-exp_trials.csv")
...
```{r random_answers}
d_overall_means.might = format_data(d.might) %>%
  group_by(modal, workerid) %>% 
  summarise(rating_m_overall = mean(rating))

d_overall_means.probably = format_data(d.probably) %>%
  group_by(modal, workerid) %>% 
  summarise(rating_m_overall = mean(rating))


d_indiv_means.might =  format_data(d.might) %>%
  group_by(modal,percentage_blue, workerid) %>% 
  summarise(rating_m = mean(rating))

d_indiv_means.probably =  format_data(d.probably) %>%
  group_by(modal,percentage_blue, workerid) %>% 
  summarise(rating_m = mean(rating))

d_indiv_merged.might = merge(d_indiv_means.might, d_overall_means.might, by=c("workerid", "modal"))

d_indiv_merged.probably = merge(d_indiv_means.probably, d_overall_means.probably, by=c("workerid", "modal"))


cors.might = d_indiv_merged.might %>%
  group_by(workerid) %>%
  summarise(corr = cor(rating_m, rating_m_overall))

cors.probably = d_indiv_merged.probably %>%
  group_by(workerid) %>%
  summarise(corr = cor(rating_m, rating_m_overall))

knitr::kable(cors.probably)

exclude.might = cors.might %>%
  filter(corr > 0.75) %>%
  .$workerid

exclude.probably = cors.probably %>%
  filter(corr > 0.75) %>%
  .$workerid


print(paste("Excluded", length(exclude.might), "participants based on random responses in might-biased condition."))
print(paste("Excluded", length(exclude.probably), "participants based on random responses in probably-biased condition."))

d.might= d.might %>% filter(!(workerid %in% exclude.might))
d.probably = d.probably %>% filter(!(workerid %in% exclude.probably)) %>% filter(workerid != 27)
...
plot_condition = function(d, plot_title) {
  ...
  group_by(workerid, percentage_blue, modal) %>% 
  ...
  colscale +
  theme(legend.position="bottom") + ggtitle(plot_title)
...
print(cohen.d(aucs.might$auc_diff, aucs.probably$auc_diff))
