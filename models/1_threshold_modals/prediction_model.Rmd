...
make_plots = function(d) {
  p1 = ggplot(d, aes(x=percentage_blue, y=rating)) + geom_point(aes(col=modal)) +  geom_smooth(aes(col=modal), method="loess") + ggtitle(d$pair[1]) + xlab("percentage") +  theme(legend.position="none")
...
```{r call, echo=FALSE}
  ...
  model_params_all = read.csv(file = "./param-estimation-output/params_all.csv")
  ...
  plot_posterior = function(modal1, modal2, exp_data, cond) {
      ...
      model_params_no_current_cond = read.csv(file = paste("./param-estimation-output/params_no_cond_",cond , ".csv", sep=""))
...
```{r fig2, fig.width=10, echo=FALSE}
