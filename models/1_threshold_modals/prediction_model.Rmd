...
```{r call, echo=FALSE}
  ...
  plot_posterior = function(modal1, modal2, exp_data) {

       data_from_R = list(modal1=modal1, modal2=modal2)
      ...
      p = ggplot(post, aes(x=percentage_blue, y=rating_pred)) + geom_smooth(method="loess", aes(col=modal)) +
          ggtitle(paste(modal1, modal2, sep="-")) + xlab("percentage")
...
for (cond in seq(0,14)) {
  ...
  post_plot = plot_posterior(pair[1], pair[2], p_data)
  data_plots = make_plots(p_data)
  
  grid.arrange(data_plots$p1, post_plot, ncol=2)
  
}  

  fname = "../../experiments/0_pre_test/data/0_pre_test-cond5_2-trials.csv"
  p_data = load_data_for_plotting(fname)
  post_plot = plot_posterior("might", "probably", p_data)
  data_plots = make_plots(p_data)
