...
class ThresholdModel:
  ...
  def listener_dist(self, utt_costs, expr_idx1, expr_idx2, rat_alpha, theta_alphas, theta_betas, utt_other_prob, noise_strength):
    speaker_dist = np.exp(self.log_speaker_dist(utt_costs, -1, -1, rat_alpha, theta_alphas, theta_betas, utt_other_prob, noise_strength))
    row_sums = np.reshape(np.sum(speaker_dist, axis=1), (len(self.expressions) + 1,1))
    return speaker_dist / row_sums
