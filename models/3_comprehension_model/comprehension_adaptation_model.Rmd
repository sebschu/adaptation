---
title: "Comprehension adaptation model"
...
compute_listener_posterior = function(theta_posterior, modal1, modal2) {
  ...
  listener_posterior <- webppl(program_file = "./comprehension_prediction_model_posterior.webppl", model_var = "model",
        inference_opts = list(method = "enumerate", verbose=TRUE), 
        data_var = "data", data = data_from_R)
  
  listener_posterior = listener_posterior[, !(names(listener_posterior) %in% c("prob"))]
  
  
  colnames(listener_posterior) <- c("percentage_blue", modal1, modal2, "bare")
  
  listener_posterior = listener_posterior %>% 
    gather("modal", "rating_pred", -percentage_blue)
      
  listener_posterior$modal = factor(listener_posterior$modal, 
                                   levels = c(modal1, modal2, "bare"), 
                                   ordered = TRUE)
  
  return(listener_posterior)
}

compute_listener_prior = function(modal1, modal2) {


  data_from_R = list(params=model_params_all[1,], modal1=modal1, modal2=modal2)       

  listener_prior <- webppl(program_file = "./comprehension_prediction_model_prior.webppl", model_var = "model",
        inference_opts = list(method = "enumerate", verbose=TRUE), 
        data_var = "data", data = data_from_R)
  
  listener_prior = listener_prior[, !(names(listener_prior) %in% c("prob"))]
  
  print(listener_prior)

  
  colnames(listener_prior) <- c("percentage_blue", modal1, modal2, "bare")
  
  listener_prior = listener_prior %>% 
    gather("modal", "rating_pred", -percentage_blue)
      
  listener_prior$modal = factor(listener_prior$modal, 
                                   levels = c(modal1, modal2, "bare"), 
                                   ordered = TRUE)
  
  return(listener_prior)
}
...
```{r call, echo=FALSE}
  ...
  model_params_all = read.csv(file = "../1_threshold_modals/param-estimation-output-100k/params_all.csv")
  
  #prior listener distributions
  
  listener_prior = compute_listener_prior(modal1 = "might", modal2 = "probably")
  
   ggplot(listener_prior, aes(x=percentage_blue, y=rating_pred)) +
 ...
 listener_posterior.probably = compute_listener_posterior(theta_posterior, "might", "probably")  

 listener_posterior.probably.means = listener_posterior.probably %>% 
    mutate(weighted_p = percentage_blue / 100 * rating_pred, weighted_p2 =  (percentage_blue / 100)^2 * rating_pred) %>% 
    group_by(modal) %>% 
   summarise(mu = sum(weighted_p), sd=sqrt(sum(weighted_p2) - sum(weighted_p)^2))

    
 p.probably = ggplot(listener_posterior.probably, aes(x=percentage_blue, y=rating_pred)) +
   geom_line(aes(col=modal)) +
   ggtitle("probably-biased") + 
   xlab("percentage") +
   theme(legend.position="none")
 ...
 listener_posterior.might = compute_listener_posterior(theta_posterior, "might", "probably")  

  listener_posterior.might.means = listener_posterior.might %>% 
    mutate(weighted_p = percentage_blue / 100 * rating_pred, weighted_p2 = (percentage_blue / 100)^2 * rating_pred) %>% 
    group_by(modal) %>% 
    summarise(mu = sum(weighted_p), sd=sqrt(sum(weighted_p2) - sum(weighted_p)^2))

    
 p.might = ggplot(listener_posterior.might, aes(x=percentage_blue, y=rating_pred)) +
   geom_line(aes(col=modal)) +
 ...
 grid.arrange(p.probably, p.might, ncol=2)
 ...
 cat("might-biased")
print(listener_posterior.might.means)

 cat("probably-biased")
 print(listener_posterior.probably.means)
  
 
 combined_tbl = merge(listener_posterior.might.means, listener_posterior.probably.means, 
                      by=c("modal"), suffixes=c(".might_biased", ".probably_biased")) %>%
   mutate(delta.mu=mu.might_biased-mu.probably_biased, 
          delta.sd=sd.might_biased-sd.probably_biased)
