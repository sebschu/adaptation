...
%DIF DEL cognition_paper_submitted.tex   Sat Jul 13 11:17:15 2019
%DIF ADD cognition_paper.tex             Wed Dec 11 13:45:27 2019
...
%DIF 41c41
%DIF < %\renewcommand{\baselinestretch}{1}
%DIF -------
\renewcommand{\baselinestretch}{1} %DIF > 
%DIF -------
...
This study was a \DIFdelbegin \DIFdel{forced-choice production experiment}\DIFdelend \DIFaddbegin \DIFadd{production expectation experiment intended to probe listeners' expectations about a generic speaker's language use}\DIFaddend .
...
Hence, a model predicting participants' beliefs about a speaker's productions of uncertainty expressions
 should  (a) be able to capture differences in ratings depending on the availability of alternative utterances \DIFdelbegin \DIFdel{; 
}\DIFdelend \DIFaddbegin \DIFadd{and
}\DIFaddend (b) provide graded predictions about utterance probabilities; 
...
For our purposes of modeling production expectations, we focus on \DIFdelbegin \DIFdel{the speaker model}\DIFdelend \DIFaddbegin \DIFadd{an a variant of the speaker agent}\DIFaddend , which crucially bases its predictions on a set of alternative utterances. 
  According to an RSA model, a speaker \DIFdelbegin \DIFdel{who }\DIFdelend \DIFaddbegin \DIFadd{agent that }\DIFaddend wants to
 convey some information to a listener \DIFdelbegin \DIFdel{chooses her }\DIFdelend \DIFaddbegin \DIFadd{agent
chooses its }\DIFaddend utterance based on the utterance's utility compared to the utility of alternative utterances. 
The speaker \DIFaddbegin \DIFadd{agent}\DIFaddend 's utterance utility  is determined by trading off the informativity of the utterance to a literal listener \DIFaddbegin \DIFadd{agent }\DIFaddend on the one hand and the cost of the utterance on the other.
...
and assume that uncertainty expressions have a threshold semantics \DIFaddbegin \parencite{Swanson2006,Yalcin2010,Lassiter2016}\DIFaddend , 
...
which is usually referred to as the \textit{literal listener} \DIFaddbegin \DIFadd{agent }\DIFaddend $L_0$ in the RSA framework. 
...
\DIFaddbegin \DIFadd{According to this formalization, a literal listener agent randomly draws an event probability $\phi$ from all values above (or, in the case of negated propositions, below) the threshold $\theta_e$ associated with the uncertainty expression $e$. }\DIFaddend $P(\phi)$ is a prior distribution over event probabilities, which is independent of the utterance by the speaker \DIFaddbegin \DIFadd{agent}\DIFaddend .

A \textit{pragmatic speaker} \DIFaddbegin \DIFadd{agent }\DIFaddend $S_1$ who wants to communicate an event probability $\phi$ then chooses \DIFdelbegin \DIFdel{her }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend utterance $u_e$ with uncertainty expression $e$ from a set of utterances $U$ according to a soft-max choice rule \parencite{Luce1959,Sutton1998} such that \DIFdelbegin \DIFdel{she }\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend chooses $u$ with a probability proportional to \DIFdelbegin \DIFdel{her }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend speaker utility. 
$$S_1\left(u_e \mid \phi, \theta, c\right) \propto exp \left( \lambda \left( \log L_0\left(\phi \mid u_e, \theta_e\right)  - c(u_e)\right)\right)$$
$\lambda$ is a rationality parameter which governs how likely a speaker \DIFaddbegin \DIFadd{agent }\DIFaddend is to choose the utterance that maximizes \DIFdelbegin \DIFdel{her }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend utility; as $\lambda$ approaches infinity, a speaker \DIFaddbegin \DIFadd{agent }\DIFaddend is more likely to always choose the optimal utterance.
\DIFaddbegin \DIFadd{In practice, when inferred from data, values for this parameter are low and generally fall within the interval $[0.5; 5]$ }\parencite[see, e.g.,][]{Herbstritt2019,Tessler2019}\DIFadd{. 
}\DIFaddend 
...
as well as  a cost function $c(u)$. The values that \DIFdelbegin \DIFdel{speakers }\DIFdelend \DIFaddbegin \DIFadd{a listener agent expects a speaker agent to }\DIFaddend assign 
...
Considering these observations,  we assume that \DIFdelbegin \DIFdel{listeners hold beliefs about speakers'}\DIFdelend \DIFaddbegin \DIFadd{the listener agent holds beliefs about a speaker agent's }\DIFaddend thresholds  in the form of a distribution $P\left(\theta_e\right)$.\footnote{We leave it open 
whether a speaker \DIFaddbegin \DIFadd{agent }\DIFaddend samples from a distribution over thresholds when making utterances (as suggested by \textcite{Qing2015}) 
or always uses the same values for thresholds. In the former case, \DIFdelbegin \DIFdel{listeners }\DIFdelend \DIFaddbegin \DIFadd{a listener agent }\DIFaddend could have higher-order beliefs  $P(\eta)$ 
about different \DIFdelbegin \DIFdel{speakers}\DIFdelend \DIFaddbegin \DIFadd{speaker agents}\DIFaddend ' threshold distributions instead of having direct beliefs about the thresholds that different 
\DIFdelbegin \DIFdel{speakers }\DIFdelend \DIFaddbegin \DIFadd{speaker agents }\DIFaddend use. For our purposes, this distinction does not matter since we would assume that \DIFdelbegin \DIFdel{listeners marginalize }\DIFdelend \DIFaddbegin \DIFadd{the listener agent marginalizes }\DIFaddend over \DIFdelbegin \DIFdel{their }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend higher-order beliefs $P(\eta)$ such that  $P\left(\theta_e\right) = \int P\left(\eta\right) P\left(\theta_e \mid \eta\right) d\eta$ and we therefore take the simplest approach and directly model $P\left(\theta_e\right)$. } Analogously, we assume that \DIFdelbegin \DIFdel{listeners also have }\DIFdelend \DIFaddbegin \DIFadd{a listener agent also has }\DIFaddend beliefs $P(c)$ about the speaker \DIFaddbegin \DIFadd{agent}\DIFaddend 's cost function.
Using these two distributions, we can define the \textit{expected pragmatic speaker} \DIFaddbegin \DIFadd{agent }\DIFaddend $ES_1\left(u_e \mid \phi \right)$ as follows:

$$ES_1\left(u_e \mid \phi \right) = \int P(c) \int_0^1 P(\theta) S_1\left(u _e\mid \phi, \theta, c\right) d\theta \  d c$$

This model predicts which utterance a listener \DIFdelbegin \DIFdel{who }\DIFdelend \DIFaddbegin \DIFadd{agent, which }\DIFaddend has uncertainty about a speaker's thresholds and cost function\DIFaddbegin \DIFadd{, }\DIFaddend would expect that speaker \DIFaddbegin \DIFadd{agent }\DIFaddend to use to describe different event probabilities.
Intuitively, this model \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{can be seen as }\DIFaddend a weighted average of different speaker models\DIFdelbegin \DIFdel{with differing }\DIFdelend \DIFaddbegin \DIFadd{, where individual speaker models differ in terms of }\DIFaddend thresholds and cost functions\DIFdelbegin \DIFdel{where the weights }\DIFdelend \DIFaddbegin \DIFadd{.  The weights of this average }\DIFaddend are determined by the 
listener \DIFaddbegin \DIFadd{agent}\DIFaddend 's belief distributions over thresholds and costs. \DIFaddbegin \DIFadd{For example, if a listener agent believes that it is likely that a speaker uses a threshold $\theta_{probably}$ of 0.5, it will assign higher weight to speaker models
...
correspond to the probabilities predicted by the \textit{expected pragmatic speaker} \DIFdelbegin \DIFdel{model}\DIFdelend \DIFaddbegin \DIFadd{agent (with the }\textit{\DIFadd{something else}}
...
For the prior distribution over event probabilities $P(\phi)$, which is used in the literal listener \DIFaddbegin \DIFadd{agent }\DIFaddend $L_0$, 
...
additionally model participant-level differences. However, since the ordering of }\textit{\DIFadd{probably}} \DIFadd{and }\textit{\DIFadd{think}} \DIFadd{is not of high relevance
...
(27 data points for each one of the 21 conditions)\DIFdelbegin \DIFdel{but to }\DIFdelend \DIFaddbegin \DIFadd{. To }\DIFaddend nevertheless rule out this possibility, we performed a leave-one-out cross-validation of
...
\DIFaddbegin \DIFadd{Further, these quantitive results inferred from generic speaker expectations are in line with qualitative accounts of the meaning of uncertainty expressions.
}\DIFaddend As we discussed above, for the bare form and its negation, we expected the model to infer threshold distributions whose probability mass is concentrated 
...
to the threshold semantics with the inferred threshold distribution of the model. The inferred threshold distribution for \textit{could} is \DIFdelbegin \DIFdel{similar }\DIFdelend \DIFaddbegin \DIFadd{highly similar, 
but not identical, }\DIFaddend to the one of \textit{might}\DIFdelbegin \DIFdel{, 
which is again in line with }\DIFdelend \DIFaddbegin \DIFadd{. This similarity largely reflects the prediction by }\DIFaddend non-probabilistic accounts, which \DIFdelbegin \DIFdel{assume that epistemic }\DIFdelend \DIFaddbegin \DIFadd{generally assume that }\DIFaddend \textit{might} and epistemic 
\textit{could} are semantically equivalent \parencite{Kratzer1991,Hacquard2011}\DIFaddbegin \DIFadd{, but it also suggests that there exist small differences between these two expressions}\DIFaddend . 

The threshold distribution for \textit{probably} has most of its probability mass concentrated at thresholds above \DIFdelbegin \DIFdel{.5}\DIFdelend \DIFaddbegin \DIFadd{0.5}\DIFaddend . This is again \DIFdelbegin \DIFdel{compatible with }\DIFdelend \DIFaddbegin \DIFadd{largely reflects the predictions of 
}\DIFaddend existing accounts that assume that \textit{probably} $p$ is true if $p$ is more likely than the negation of $p$ \parencite[e.g.,][]{Kratzer1991}. However, it is also noteworthy, that the inferred
...
uncertainty expressions for a short period of time. The procedure, materials and analyses were pre-registered at \url{https://osf.io/w926x/}.\DIFaddbegin \footnote{\DIFadd{This experiment is a follow-up to a previous experiment with a potential confound due to different number of exposures across conditions. See Appendix~E for a discussion of the previous experiment.}}
\DIFaddend 
...
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Participants}}
\DIFaddend We recruited a total of 80 participants (40 per condition) on Amazon Mechanical Turk. 
...
\$12--\$15. None of the participants had \DIFdelbegin \DIFdel{previously }\DIFdelend participated in Experiment~1.
...
%\begin{table}
%\centering
%\DIFdelbeginFL %DIFDELCMD < \begin{tabular}{l c c c c c c | c c c c c c}
%%DIFDELCMD < %%%
%\DIFdelendFL \DIFaddbeginFL \begin{tabular}{l c c c c c c}
%\DIFaddendFL \toprule
%& \DIFdelbeginFL %DIFDELCMD < \multicolumn{6}{c | }{Experiment 2a} & \multicolumn{6}{c}{Experiment 2b} \\
%%DIFDELCMD < \midrule
%%DIFDELCMD < & %%%
%\DIFdelendFL \multicolumn{2}{c}{\sc might} & \multicolumn{2}{c}{\sc probably} & \DIFdelbeginFL %DIFDELCMD < \multicolumn{2}{c |}{\sc bare} & \multicolumn{2}{c}{\sc might} & \multicolumn{2}{c}{\sc probably} & \multicolumn{2}{c}{\sc bare} %%%
%\DIFdelendFL \DIFaddbeginFL \multicolumn{2}{c }{\sc bare}  \DIFaddendFL \\
%& $n$ & $\phi$ & $n$ & $\phi$ & $n$ & $\phi$ \DIFdelbeginFL %DIFDELCMD < & %%%
%\DIFdelFL{$n$ }%DIFDELCMD < & %%%
%\DIFdelFL{$\phi$ }%DIFDELCMD < & %%%
%\DIFdelFL{$n$ }%DIFDELCMD < & %%%
%\DIFdelFL{$\phi$ }%DIFDELCMD < & %%%
%\DIFdelFL{$n$ }%DIFDELCMD < & %%%
%\DIFdelFL{$\phi$}\DIFdelendFL \\
%\midrule
%\emph{cautious} & {\bf 10} & {\bf 60\%} & \DIFdelbeginFL \DIFdelFL{5 }%DIFDELCMD < & %%%
%\DIFdelFL{90\% }%DIFDELCMD < & %%%
%\DIFdelFL{5 }%DIFDELCMD < & %%%
%\DIFdelFL{100\% }%DIFDELCMD < & {\bf %%%
%\DIFdelendFL 10 \DIFdelbeginFL %DIFDELCMD < } %%%
%\DIFdelendFL & \DIFdelbeginFL %DIFDELCMD < {\bf %%%
%\DIFdelFL{60\%}%DIFDELCMD < } & %%%
%\DIFdelFL{10 }%DIFDELCMD < & %%%
%\DIFdelendFL 90\% & 5 & 100\%  \\
%\emph{confident} & \DIFdelbeginFL \DIFdelFL{5 }%DIFDELCMD < & %%%
%\DIFdelFL{25\% }%DIFDELCMD < & {\bf %%%
%\DIFdelendFL 10 \DIFdelbeginFL %DIFDELCMD < }  %%%
%\DIFdelendFL & \DIFdelbeginFL %DIFDELCMD < {\bf %%%
%\DIFdelFL{60\%}%DIFDELCMD < } & %%%
%\DIFdelFL{5  }%DIFDELCMD < & %%%
...
%\DIFdelFL{10 }%DIFDELCMD < & %%%
%\DIFdelendFL 25\% & {\bf 10}  & {\bf 60\%} & 5  & 100\%  \\  
%\bottomrule
%\end{tabular}
...
Our results further suggest that participants updated their mappings between uncertainty expressions and event probabilities: In the {\it confident speaker} condition, {\sc might} and {\sc probably} were rated highly for lower event probabilities than in the {\it cautious speaker} condition. \DIFdelbegin \DIFdel{However, one potential confound in this experiment is that the number of utterances with }\textit{\DIFdel{might}} %DIFAUXCMD
...
%DIF > That we nevertheless observed different ratings in the two speaker conditions suggests that participants updated their beliefs about the mapping between expression and event probabilities.
\DIFaddbegin 
...
makes sense nor is possible in our current framework to estimate this quantity and hence we also cannot compute a meaningful Bayesian $R^2$ distribution.}} 
\DIFadd{For these reasons, we present $R^2$ values only to provide intuitions about how well the model predictions match the empirical ratings but for the formal model comparisons, we 
...
In the previous two sections, we presented the results from \DIFdelbegin \DIFdel{two experiments, which }\DIFdelend \DIFaddbegin \DIFadd{an experiment that }\DIFaddend provide strong evidence for
...
listeners also update }\DIFdelend beliefs about speaker preferences.
...
If participants update their expectations of a specific speaker's use of uncertainty expressions,  we expect that \DIFdelbegin \DIFdel{listeners }\DIFdelend \DIFaddbegin \DIFadd{participants }\DIFaddend interpret a more confident speaker's utterance 
...
\DIFaddbegin \DIFadd{Here, we make the implicit linking assumption that the distribution of coins reflects participants' interpretations, i.e., their beliefs about the event likelihood after hearing an utterance.
The choice for this paradigm, which is very similar to betting paradigms that have been used to study interpretations of utterances in reference games }\parencite{Frank2012,Goodman2013} \DIFadd{as well as for probing subjective probabilities }\parencite[e.g.,][]{Hampton1973}\DIFadd{, was motivated by the assumption that listeners will have uncertainty about the exact interpretation after hearing }\textsc{\DIFadd{might}} \DIFadd{or }\textsc{\DIFadd{probably}}\DIFadd{. Allowing
...
participants behaved as we intended: they assigned almost all coins to the gumball machine with 100\% target color gumballs after hearing }\textsc{\DIFadd{bare}}\DIFadd{, 
an utterance about whose interpretation participants likely have very little uncertainty, whereas participants assigned coins to multiple machines after hearing }\textsc{\DIFadd{might}} \DIFadd{or }\textsc{\DIFadd{probably}}\DIFadd{. 
}\DIFaddend 
...
\DIFdelbegin \DIFdel{While adaptation in language is a widely attested phenomenon, the exact nature of the representations that are updated during semantic/pragmatic adaptation has remained largely a mystery. In this paper we attempted to rectify this situation by doing three things: first, to investigate whether semantic/pragmatic adaptation occurs at all in a novel domain, that of uncertainty expressions;  second, to investigate the nature of the representations that are updated during adaptation via the comparison of computational models of adaptation formulated within a Bayesian pragmatic modeling framework; and third, to test a novel prediction resulting from the application of this model to interpretation.  
}\DIFdelend %DIF > While adaptation in language is a widely attested phenomenon, the exact nature of the representations that are updated during semantic/pragmatic adaptation has remained largely a mystery. In this paper we attempted to rectify this situation by doing three things: first, to investigate whether semantic/pragmatic adaptation occurs at all in a novel domain, that of uncertainty expressions;  second, to investigate the nature of the representations that are updated during adaptation via the comparison of computational models of adaptation formulated within a Bayesian pragmatic modeling framework; and third, to test a novel prediction resulting from the application of this model to interpretation.  

In \DIFdelbegin \DIFdel{two production expectation experiments (Experiments~2a and 2b}\DIFdelend \DIFaddbegin \DIFadd{a production expectation experiment (Experiment~2}\DIFaddend ), we found that listeners adapt to speakers who vary in their use of uncertainty expressions. This result confirms that 
...
many cognitive processes can be seen as an instance of integrating prior beliefs with observations}\DIFaddend . 

%DIF > TODO: is there something we can say about a shared cognitive process?
...
\DIFaddend At the semantic and pragmatic level, \textcite{Hawkins2017} proposed a model 
...
Yet we did. }\DIFdelend \DIFaddbegin 
\DIFadd{appears to be compatible with our results. However, considering that Yildirim et al. (2016) and 
Schuster and Degen (2019) found that listeners form speaker-specific expectations rather than expecting speakers
...
}\DIFdelend \DIFaddbegin \DIFadd{participants did not engage
in }\DIFaddend any non-linguistic task\DIFdelbegin \DIFdel{, since participants only listened to pre-recorded utterances during the exposure phase. While this }\DIFdelend \DIFaddbegin \DIFadd{. While we tried to make the situation in the experiments natural,
...
to what extent the results in the present research translate to less scripted and more interactive settings }\DIFaddend is an important area for future research.
...
\subsection*{E \DIFdelbegin \DIFdel{Model simulations for Experiment~2b}\DIFdelend \DIFaddbegin \DIFadd{Original production expectation adaptation experiment}\DIFaddend }

\DIFdelbegin \DIFdel{Figures~\ref{fig:post-exposure-model-replication}, \ref{fig:post-exposure-thresholds-replication}, and \ref{fig:post-exposure-costs-replication} }\DIFdelend \DIFaddbegin \DIFadd{We originally ran a different version of the production expectation experiment which included a potential confound because the number of utterances
...
\DIFadd{For transparency, we report the procedure and the results of the original experiment here. The procedure, materials and analyses were pre-registered at }\url{https://osf.io/w926x/}\DIFadd{.
}

\subsection*{\DIFadd{Participants}}
...
\subsection*{\DIFadd{Materials and procedure}}
...
\subsubsection{\DIFadd{Results and discussion}}
...
\subsection*{\DIFadd{F Model simulations for original production expectation experiment}}
...
\subsection*{\DIFdelbegin \DIFdel{F }\DIFdelend \DIFaddbegin \DIFadd{G }\DIFaddend Original interpretation experiment}
