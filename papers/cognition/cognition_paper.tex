...
They consist of listener and speaker models which recursively reason about each other to derive interpretations and choose utterances. 
For our purposes of modeling production expectations, we focus on an a variant of the speaker model, which crucially bases its predictions on a set of alternative utterances. 
  According to an RSA model, a speaker model that wants to
 convey some information to a listener model
chooses its utterance based on the utterance's utility compared to the utility of alternative utterances. 
The speaker's utterance utility is determined by trading off the informativity of the utterance to a literal listener model on the one hand and the cost of the utterance on the other.
...
``It might rain this afternoon'' is semantically felicitous to a speaker model that believes that the probability of rain in the afternoon exceeds $0.1$.
...
which is usually referred to as the \textit{literal listener} model $L_0$ in the RSA framework. 
...
According to this formalization, the literal listener model randomly draws an event probability $\phi$ from all values above  the threshold $\theta_e$ associated with the uncertainty expression $e$ (or, in the case of negated propositions, from all values below the threshold $\theta_e^{'}$). $P(\phi)$ is a prior distribution over event probabilities, which is independent of the observed utterance.

A \textit{pragmatic speaker} model $S_1$ that wants to communicate an event probability $\phi$ then chooses an utterance $u_e$ with uncertainty expression $e$ from a set of utterances $U$ according to a soft-max choice rule \parencite{Luce1959,Sutton1998} such that $u$ is chosen with a probability proportional to its utility for the speaker:
$$S_1\left(u_e \mid \phi, \theta, c\right) \propto exp \left( \lambda \left( \log L_0\left(\phi \mid u_e, \theta_e\right)  - c(u_e)\right)\right)$$
$\lambda$ is a rationality parameter which governs how likely a speaker model is to choose the utterance that maximizes utility. As $\lambda$ approaches infinity, a speaker model is more likely to always choose the optimal utterance. As it approaches 0, the speaker produces true utterances at random. In practice, when inferred from data, values for this parameter often fall within the interval $[0.5; 5]$ \parencite[see, e.g.,][]{Herbstritt2019,Tessler2019}. \jd{but interpreting the absolute value of the parameter is meaningless} \seb{Yes, but that's what a reviewer wanted. TO DISCUSS.}
...
as well as  a cost function $c(u)$. The values that a listener model expects a speaker model to assign 
...
Considering these observations,  we assume that the listener model holds beliefs about a speaker model's thresholds  in the form of a distribution $P\left(\theta_e\right)$.\footnote{We leave it open 
whether a speaker model samples from a distribution over thresholds when producing utterances (as suggested by \textcite{Qing2015}) 
or always uses the same values for thresholds. In the former case, a listener model could have higher-order beliefs  $P(\eta)$ 
about different speaker models' threshold distributions instead of having direct beliefs about the thresholds that different 
speaker models use. For our purposes, this distinction does not matter since we assume that the listener model marginalizes over higher-order beliefs $P(\eta)$ such that  $P\left(\theta_e\right) = \int P\left(\eta\right) P\left(\theta_e \mid \eta\right) d\eta$ and we therefore take the simplest approach and directly model $P\left(\theta_e\right)$. } Analogously, we assume that a listener model also has beliefs $P(c)$ about the speaker model's cost function.
...
This model predicts which utterance a listener model with uncertainty about a speaker's thresholds and cost function would expect that speaker model to use to describe different event probabilities.
...
listener model's belief distributions over thresholds and costs. For example, if a listener model believes that it is likely that a speaker uses a threshold $\theta_{probably}$ of 0.5, it will assign higher weight to speaker models
...
many cognitive processes can be seen as an instance of integrating prior beliefs with observed signals from the environment.
...
such as ``It will probably rain today'', over the course of the experiment, participants would increase fixations on the visual representation of an event 
