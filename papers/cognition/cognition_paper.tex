...
\abstract{
Pragmatic theories of interpreting utterances in context generally make
the assumption that listeners reason about alternative utterances that a
speaker could have said to describe different situations. Such reasoning
necessitates precise expectations about a speaker's productions
in different contexts, which is at odds with the considerable variability across speakers that 
exists at all levels of linguistic representation. Listeners seem reconcile
this tension by adapting to the statistics of an individual speaker. In this work,
we investigate this kind of semantic/pragmatic adaptation in the domain of
...
change after being exposed to a specific speaker. We further investigate,
the representations that listeners update during interpretation. We present
a novel computational model of the adaptation process based on Bayesian
belief updating and through a series of model comparisons, we find that
semantic adaptation is a product of listeners updating beliefs about a
speaker's lexicon as well as a speaker's utterance preferences. 
}
...
favor simpler models. For a more complex model with more parameters, the distribution over different parameter values will be more dispersed and can contain more parameter configurations that lead to a lower likelihood of the data.
...
\subsection*{B Additional results from Eperiment~1.}
...
\caption{Model predictions and results from Experiment~1 -- Part 1. Error bars correspond to 95\% high density intervals (model predictions) and bootstrapped 95\%-confidence intervals (observed results). \label{fig:norming-results-model-1}}

\end{figure}

\begin{figure}[h!]
\includegraphics[width=0.95\textwidth]{plots/pre\string_test\string_model\string_s2.pdf}
\caption{Model predictions and results from Experiment~1  -- Part 2. Error bars correspond to 95\% high density intervals (model predictions) and bootstrapped 95\%-confidence intervals (observed results). \label{fig:norming-results-model-2}}
...
As we mentioned in the main text, we originally ran a slightly different version of the comprehension experiment in which participants were using sliders to rate which gumball machine they though the speaker saw when he or she produced an utterance. The use of sliders seemed to confuse some participants and therefore we changed the procedure such that participants provided ratings by distributing coins. For the sake of completeness, we report the 
...
Qualitatively, the results are the same as in Experiment~3. However, since participants had the option to assign high 
ratings (they could assign a maximum rating to each gumball if they wanted to) to 
all gumball machines, we noticed that many participants assigned very high ratings to most gumball 
machines and therefore did not indicate their interpretation of the utterance. Further, it seemed that some participants
understood the instructions as rating the likelihood of getting a blue gumball and provided ratings proportional to the 
