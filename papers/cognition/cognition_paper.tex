...
\keywords{adaptation; language comprehension; experimental pragmatics; Bayesian cognitive modeling}
...
As a starting point for this investigation, we consider adaptation in other linguistic domains. Apart from the work on quantifiers, linguistic adaptation has been observed in  phonetics \parencite[e.g.,][]{Goldinger1998,Norris2003,Kraljic2005,Kraljic2007,Babel2012,Kleinschmidt2015,Clayards2008}, 
...
\parencite{Clark1986,Brennan1996,Metzing2003,Horton2005,Brennan2009,Hawkins2017}, contrastive inferences \parencite{Grodner2011,kurumada or pogue?}, and lexical associations \parencite{DelaneyBusch2019}. 
...
The inclusion of the noise term is not crucial for fitting the data but it does improve model fit \jd{isn't it necessary?? maybe also cite michael henry's 2019 paper to show that there's precedent for this and isn't just sth you did randomly to make the model do better.}.
...
that speakers use \textit{think} when they strongly believe the embedded proposition but are not entirely certain that it is true.
 ...
 \todo{Include and discuss MAP estimates of other parameters.} \jd{is this where you were just going to add the table? that seems reasonable -- i don't think it's worth discussing too much more because it'll get extremely long. i have a feeling that reviewer's will already recommend putting a lot of the model description (ie the linking assumption section) into the Supp Info. but we should put it out like this first instead of relegating too much to the SI}
...
models of adaptation and then discuss what the results tell us about existing mechanistic accounts of
 interactive language processing. \jd{"adaptation" insntead of "interactive language processing"?}
...
adaptation \parencite{Kleinschmidt2012}, and adaptation in the interpretation of prosodic cues \parencite{Roettger2019}.  \jd{this would be a good place to mention ciyang and robert's workj (can just add citations to the sentence) -- oh now i see that robert is in there later. but definitely also include ciyang, given that his is the model that's closest to yours.}
...
suggests that the semantics of these expressions is highly dynamic and context-sensitive. This is, to some extent, predicted by recent 
...
state that the meanings of probability operators are highly dynamic and largely determined by the context. However, these accounts
generally limit this dynamicity to the meaning of probability operators and exclude other uncertainty expressions such as \textit{might}. Moreover,  according to these accounts the main source of variability is the prior probability of the event embedded under a probability operator. 
However, in the above reported experiments we did not observe a difference between the probability operator \emph{probably} and other types of uncertainty expressions like \emph{might}; listeners readily updated their expectations about the use of both expressions. \jd{i'm not sure we should be arguing against these kinds of accounts (which is what it currently sounds like), given that this is a development in semantics that we should be supporting -- our findings just say that this dynamic approach should be extended further beyond probability operators and beyond the single source of variability being prior event probability; ie verbiage like ``even more dynamic than proposed by these approaches'' may be appropriate}
This suggests that different types of uncertainty expressions have a dynamic meaning whose interpretation depends on contextual factors.
We further found that after being exposed to a specific speaker, listeners updated both their production expectations and their interpretations, which suggests
that the prior probability of an event is not the only contextual factor that influences the interpretation of uncertainty expressions,
...
production expectations or interpretations of uncertainty expressions (see also \textcite{Jaeger2010}). 
...
alternatives allows for inferring the latent meanings of utterances by combining data from different experiments (see also \textcite{Franke2014} and \textcite{Peloquin2016} for a similar approach).
...
makes concrete predictions to test in future experiments. For example, one
prediction of the proposed model is that the number of exposure trials should influence the size of the
adaptation effect. Some evidence in support of this prediction is reported by \textcite{Schuster2019}, but a systematic investigation of  how the amount of exposure to a specific speaker affects adaptation remains to be conducted. \jd{i wonder whether this is a prediction that's specific enough to the model, given that most low-level priming effects would presumably make the same prediction?}
...
has an incentive to be encouraging or has additional goals besides being informative (see also \textcite{Yoon2016,Yoon2017}). Investigating whether listeners draw such complex inferences
