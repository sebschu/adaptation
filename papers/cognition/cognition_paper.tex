...
\parencite{Clark1986,Brennan1996,Metzing2003,Horton2005,Brennan2009}, contrastive inferences \parencite{Grodner2011}, and lexical associations \parencite{DelaneyBusch2019}. 
...
that speakers use \textit{think} when they strongly belief in the embedded proposition but are not entirely certain that the proposition is true.
 ...
 \todo{Include and discuss MAP estimates of other parameters.}
...
Our results also have implications for theories about the semantics of uncertainty expressions.
...
and according to these accounts the main source of variability is the prior probability of the event embedded under a probability operator.
In our experiment, on the other hand, we did not find any differences between probability operators and other types of uncertainty expressions;
for example, listeners also readily updated their expectations about the use of \textit{might}. 
...
but rather that other factors such as the speaker identity also influence interpretations.
...
our findings that listeners adapt to the statistics of their environment within a short experiment,
imply that experimenters have to be cognizant of potential adaptation effects when probing
production expectations or interpretations of uncertainty expressions.

Further, the results from Experiment~1, and in particular, the finding
that participant's ratings for the utterances in the experiment strongly depended on
the alternative utterances that we provided, highlights that one has to be very careful from 
drawing conclusions about the use of utterances from a single experiment. For example,
had we only considered the results from the \textit{bare-might} condition (see \figref{ig:norming-results-main}),
we could have concluded that ``might'' is an appropriate expression to communicate an event probability of 75\%,
whereas we would have found the opposite if we had only considered the results from the \textit{might-probably} condition.
At the same time, our modeling results show, that if one has a concrete linking function which models the effects of 
alternatives, one can infer the latent meanings of utterances by combining data from different experiments.
...
While this is clearly different from everyday dialogue, it mirrors other everyday situations such as listening to someone 
...
if it had been relevant for a task. Nevertheless, investigating semantic/pragmatic adaptation in a less scripted setting is 
an important area for future research.


Throughout this paper, we made the assumption that listeners are forming \textit{speaker-specific} production expectations. However,
since all our experiments had a between-subjects design, it could also be that participants were only adapting to the experimental
situation independent of the speaker. This seems unlikely given the results by \textcite{Yildirim2016} who found that
participants formed speaker-specific production expectations after being exposed to multiple speakers whose use of
quantifiers differed. On top of that, we have preliminary results, which suggest that speaker-specific adaptation can also be observed with 
uncertainty expressions \parencite{Schuster2019}, but which aspects about a situation (e.g., the speaker, the topic of conversation, the visual context, etc.)
listeners are adapting to is still not entirely clear and should be investigated further.


Further, we only investigated adaptation to uncertainty expressions in English 
...
One advantage of formalizing a theory as a computational model is that the model also
makes concrete predictions which can be tested in future experiments. For example, one
prediction of our model is that the number of exposure trials should influence the size of the
adaptation effect. We again have preliminary results that suggest that this relationship holds \parencite{Schuster2019},
but yet another future direction would be to systematically investigate how the amount of exposure to a specific speaker
influences the extent of adaptation.
...
wants to be very encouraging or has some other goal independent of being informative. Investigating whether listeners draw such complex inferences
could provide us with insights about which kind of speaker goals listeners are reasoning and about what kind of speaker goals affect adaptation.


\subsection{Conclusion}

In a series of web-based experiments, we found that after exposure to a specific speaker, listeners rapidly update 
their expectations about which uncertainty expressions that speaker would produce
in different situations and that these updated expectations also transferred to updated interpretations.
We provided a first formal account of semantic/pragmatic adaptation and modeled this behavior 
using a Bayesian cognitive model which suggests a communicatively efficient system.
