...
and assume that uncertainty expressions have a threshold semantics \parencite{Swanson2006,Yalcin2010,Lassiter2016}, 
...
Further, these quantitive results inferred from generic speaker expectations are in line with qualitative accounts of the meaning of uncertainty expressions.
...
to the threshold semantics with the inferred threshold distribution of the model. The inferred threshold distribution for \textit{could} is highly similar, 
but not identical, to the one of \textit{might}. This similarity largely reflects the prediction by non-probabilistic accounts, which generally assume that \textit{might} and epistemic 
\textit{could} are semantically equivalent \parencite{Kratzer1991,Hacquard2011}, but it also suggests that there exist small differences between these two expressions. 

The threshold distribution for \textit{probably} has most of its probability mass concentrated at thresholds above 0.5. This is again largely reflects the predictions of 
...
For these reasons, we present $R^2$ values only to provide intuitions about how well the model predictions match the empirical ratings but for the formal model comparisons, we 
...
In a production expectation experiment (Experiment~2), we found that listeners adapt to speakers who vary in their use of uncertainty expressions. This result confirms that 
