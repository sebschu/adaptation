...
For our purposes of modeling production expectations, we focus on a model of listener beliefs about a speaker's production model.
  According to an RSA model, a {speaker} that wants to
...
``It might rain this afternoon'' is semantically felicitous to a {speaker} who believes that the probability of rain in the afternoon exceeds $0.1$.
...
The absolute value of this parameter is influenced by the literal listener distributions and the cost function and therefore it is complex to determine a reasonable range for this parameter a priori, except that the value should be greater than 0. However, \textcite{Herbstritt2019} inferred $\lambda$ from data for a similar model as ours and found it to be likely in the interval $[4.5;5;2]$, which suggests that we should also expect a similarly large value for $\lambda$ in our setup.} 
