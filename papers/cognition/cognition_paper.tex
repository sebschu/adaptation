...
$P(\phi)$ is a prior distribution over event probabilities, which is independent of the utterance by the speaker agent.
...
Intuitively, this model is a weighted average of different speaker models with differing thresholds and cost functions where the weights are determined by the listener agent's belief distributions over thresholds
and costs. 
...
correspond to the probabilities predicted by the \textit{expected pragmatic speaker} agent.
